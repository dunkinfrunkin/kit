...
func (s *Server) handleSSORedirect(w http.ResponseWriter, r *http.Request) {
	...
	params := fmt.Sprintf("%s?client_id=%s&response_type=code&scope=openid+email+profile&redirect_uri=%s&state=%s",
		authEndpoint,
		cfg.ClientID,
		callbackURL,
		state,
...
func (s *Server) handleSSOCallback(w http.ResponseWriter, r *http.Request) {
	...
	// Return HTML that stores the token and redirects to /ui
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	fmt.Fprintf(w, `<!DOCTYPE html><html><head><meta charset="utf-8"><title>kit — logged in</title></head>
